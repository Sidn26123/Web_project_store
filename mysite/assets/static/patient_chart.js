...
$(document).ready(function () {
    $.ajax({
        ...
        success: function (data) {
            ...
            $('#gender-proportion-chart').height = 300;
...
function get_status_update(){
    $.ajax({
        ...
        success: function (data) {
            ...
            console.log(data)
            $('.total-acc-normal').find('span').text(data.amounts[0]);
            $('.total-acc-locked').find('span').text(data.amounts[1]);
        }
...
function update_amount_info(){
    $.ajax({
        url: "/ad/get-amount-info/",
        type: 'GET',
        success: function(data){
            console.log(data['not_book_yet'])
            $('.accounts').find('span').text(data['accounts'])
            $('.not-booked-yet').find('span').text(data['not_book_yet'])
            $('.at-least-one').find('span').text(data['as_least_one'])
            $('.more-than-one').find('span').text(data['more_than_one'])
            $('.patient-satisfied').find('span').text(data['patient_satisfied'])
        }
    })
}