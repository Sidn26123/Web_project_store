...
$(document).ready(function(){
    ...
    $(document).on('click', '.item-chose-delete-btn', function(){
        ...
        $('#'+id).closest('.search-filter__item').removeClass('checkbox-custom-checked'); //Thêm class vào node hiện tại click

        $('#'+id).prop('checked', false); //Chuyển trạng thái checked của node input (con của this) thành true
        console.log($('#'+id).prop('checked'))
    });
});
